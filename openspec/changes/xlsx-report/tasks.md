...
- [x] 1.1 Add `openpyxl` to `pyproject.toml` runtime dependencies and reinstall;
  verify `import openpyxl` succeeds in the project venv
- [x] 1.2 Rewrite `write_report()` in `report.py` to build an `.xlsx` workbook:
...
- [x] 1.3 Verify the no-duplicates case still produces a valid, openable workbook
  with no group tables; verify with a unit test (already covered by
  test_write_report_with_no_groups_creates_valid_empty_workbook added in 1.2)

## 2. Wiring and existing tests

- [x] 2.1 Update `cli.py` to write `duplicates.xlsx` instead of `duplicates.csv`;
...
- [x] 2.2 Update `tests/test_report.py` and `tests/test_cli.py` to assert against
  the new `.xlsx` output instead of CSV rows; verify the full test suite passes
- [x] 2.3 Update `README.md`'s demo description from `duplicates.csv` to
