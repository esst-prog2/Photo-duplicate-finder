...
- [x] 1.1 In `cli.py`, build the output path as
...
- [x] 1.2 Run `find-duplicates tests/fixtures/` and confirm the report file is
  created with a timestamped name next to the folder

## 2. Overwrite confirmation

- [x] 2.1 Before writing, check whether a file already exists at the computed
...
- [x] 2.2 Verify with a unit test that no prompt appears when the target filename
  does not already exist

## 3. Update existing tests and docs

- [x] 3.1 Update `tests/test_cli.py` assertions that assume the fixed
  `duplicates.xlsx` filename to work with the new timestamped name; verify the
  full test suite passes
- [x] 3.2 Update `README.md`'s demo description to reflect the timestamped
